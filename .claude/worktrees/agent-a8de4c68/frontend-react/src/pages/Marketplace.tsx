...
export default function Marketplace() {
  ...
  async function togglePyme(p: Pyme) {
    try {
      await Api.marketplaceActualizarPyme(p.id, { activo: !Boolean(p.activo) });
      await loadBase();
    } catch (e: any) {
  ...
  async function toggleOferta(o: Oferta) {
    try {
      await Api.marketplaceActualizarOferta(o.id, { activo: !Boolean(o.activo) });
      await loadOfertas(selectedAlianzaId);
    } catch (e: any) {
          ...
          <tbody className="text-slate-200">
            {(loading ? [] : pymes).map((p) => (
              <tr key={p.id} className="border-t border-white/10 hover:bg-white/5">
                ...
                <td className="py-2 px-2">{Boolean(p.activo) ? 'Activa' : 'Inactiva'}</td>
                <td className="py-2 px-2">
                  ...
                  >
                    {Boolean(p.activo) ? 'Desactivar' : 'Activar'}
                  </button>
                </td>
              </tr>
            ))}
