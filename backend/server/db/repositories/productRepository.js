...
const { roundPrice, normalizeStep } = require('../../lib/priceUtils');
...
async function listProducts({
  ...
  if (tipo) {
    params.push(tipo);
    where.push(`p.tipo_producto = $${params.length}`);
  }
  if (q) {
  ...
  if (stockFilter === 'nulo') {
    where.push(`COALESCE(i.cantidad_disponible, 0) = 0`);
  } else if (stockFilter === 'bajo') {
    where.push(`COALESCE(i.cantidad_disponible, 0) > 0 AND COALESCE(i.cantidad_disponible, 0) <= COALESCE(p.stock_minimo, 0)`);
  } else if (stockFilter === 'ambos') {
    where.push(`COALESCE(i.cantidad_disponible, 0) <= COALESCE(p.stock_minimo, 0)`);
  }
  ...
  if (depositoId != null) {
    ...
    if (Number.isInteger(depId) && depId > 0) {
      params.push(depId);
      joinInventario = `LEFT JOIN inventario_depositos i ON i.producto_id = p.id AND i.deposito_id = $${params.length}`;
    }
  }
...
async function createProduct({
  ...
  try {
    roundStep = normalizeStep(await configRepo.getPriceRoundingStep());
  } catch {
    // Si falla, usamos step=1 (entero sin decimales)
  }
  ...
  const computedPrecioLocal = roundPrice(
    costoPesosFinal > 0 ? costoPesosFinal * (1 + margenLocal) : basePrecioVenta || 0,
    roundStep
  );
  const computedPrecioDistribuidor = roundPrice(
    costoPesosFinal > 0 ? costoPesosFinal * (1 + margenDistribuidor) : basePrecioVenta || 0,
    roundStep
  );

  const precioLocal =
    priceMode === 'manual'
      ? (hasManualLocal ? roundPrice(manualPrecioLocal, roundStep) : 0)
      : computedPrecioLocal;
  const precioDistribuidor =
    priceMode === 'manual'
      ? (hasManualDistribuidor ? roundPrice(manualPrecioDistribuidor, roundStep) : 0)
      : computedPrecioDistribuidor;
  const precioVentaFinal =
    priceMode === 'manual'
      ? (hasManualLocal ? roundPrice(manualPrecioLocal, roundStep) : roundPrice(basePrecioVenta || precioLocal || 0, roundStep))
      : computedPrecioLocal;
  const precioFinalManual =
    typeof precio_final !== 'undefined' && Number.isFinite(Number(precio_final))
      ? roundPrice(Number(precio_final) || 0, roundStep)
      : 0;
  ...
  return withTransaction(async (client) => {
    ...
    const ins = await client.query(
      ...
      [
        ...
        proveedor_id || null,
        ['estandar','insumo','servicio'].includes(tipo_producto) ? tipo_producto : 'estandar',
      ]
    );
    ...
    await priceListRepo.syncProductPriceRowsTx(client, productoId, {
      motivo: 'create_product',
      recordHistory: false,
    });
...
async function updateProduct(
  ...
  return withTransaction(async (client) => {
    ...
    if (typeof precio_final !== 'undefined') {
      sets.push(`precio_final = $${p++}`);
      params.push(roundPrice(Number(precio_final) || 0, roundStep));
    }
    ...
    if (typeof tipo_producto !== 'undefined' && ['estandar','insumo','servicio'].includes(tipo_producto)) {
      sets.push(`tipo_producto = $${p++}`);
    ...
    if (priceMode === 'manual') {
      if (hasManualLocal) {
        ...
        params.push(roundPrice(manualLocal, roundStep));
      }
      if (hasManualDistribuidor) {
        sets.push(`precio_distribuidor = $${p++}`);
        params.push(roundPrice(manualDistribuidor, roundStep));
      }
      if (hasPrice) {
        sets.push(`precio_venta = $${p++}`);
        params.push(roundPrice(Number(price), roundStep));
      ...
      if (costoPesosFinal > 0) {
        precioLocalFinal = roundPrice(costoPesosFinal * (1 + margenLocal), roundStep);
        precioDistribuidorFinal = roundPrice(
          costoPesosFinal * (1 + margenDistribuidor),
          roundStep
        );
      } else if (typeof price !== 'undefined') {
        precioLocalFinal = roundPrice(Number(precioVentaFinal || 0), roundStep);
        precioDistribuidorFinal = roundPrice(Number(precioVentaFinal || 0), roundStep);
      }
      ...
      if (typeof precioVentaFinal !== 'undefined') {
        sets.push(`precio_venta = $${p++}`);
        params.push(roundPrice(Number(precioVentaFinal || 0), roundStep));
      }
    ...
    await priceListRepo.syncProductPriceRowsTx(client, id, {
      motivo: 'update_product',
      recordHistory: false,
    });
