...
async function createCompra({ proveedor_id, fecha, moneda = 'USD', detalle = [], oc_numero, adjunto_url }) {
  return withTransaction(async (client) => {
    ...
    try {
      await companyAccountRepo.ensureCompraMovement(
        {
          compra_id: compraId,
          proveedor_id,
          monto: total,
          descripcion: `Compra #${compraId}${oc_numero ? ` - OC ${oc_numero}` : ''}`,
        },
        client
      );
    } catch (error) {
...
async function recibirCompra({ compra_id, fecha_recepcion, observaciones, usuario_id, deposito_id, detalle }) {
  return withTransaction(async (client) => {
    ...
    for (const d of toReceive) {
      ...
      const priceMode =
        String(prod.precio_modo || '').trim().toLowerCase() === 'manual' ? 'manual' : 'auto';
      ...
      const precioLocal = costoPesos > 0 ? roundPrice(costoPesos * (1 + margenLocal), roundStep) : 0;
      const precioDistribuidor = costoPesos > 0 ? roundPrice(costoPesos * (1 + margenDistribuidor), roundStep) : 0;
      ...
      await priceListRepo.syncProductPriceRowsTx(client, d.producto_id, {
        motivo: 'receive_purchase',
      ...
      await client.query(
        ...
        [
          ...
          costoDolares || null,
          tipoCambio,
          Number(synced.margen_local) || margenLocal,
          Number(synced.margen_distribuidor) || margenDistribuidor,
          Number(synced.precio_local) || null,
          Number(synced.precio_distribuidor) || null,
          usuario_id || null,
        ]
      );
    }
