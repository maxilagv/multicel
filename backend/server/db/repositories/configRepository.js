...
async function getPriceRoundingStep() {
  const val = await getNumericParam('precio_redondeo_step');
  return val != null ? val : 1;
}

async function setPriceRoundingStep(step, usuarioId) {
  return setNumericParam('precio_redondeo_step', step, usuarioId);
}

async function getModulesConfig() {
  const val = await getTextParam('modules_config');
  if (!val) return {};
  try {
...
async function setModulesConfig(config, usuarioId) {
  return setTextParam('modules_config', JSON.stringify(config), usuarioId);
}
...
module.exports = {
  ...
  getNetworkSubnet,
  setNetworkSubnet,
  getPriceRoundingStep,
  setPriceRoundingStep,
  getModulesConfig,
  setModulesConfig,
};
