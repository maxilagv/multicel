...
const { buildClientVisibilityClause } = require('../../lib/clientVisibility');
...
async function listDebtRiskBase({ limit = 100, clientVisibility = null } = {}) {
  const lim = Math.min(Math.max(parseInt(limit, 10) || 100, 1), 500);
  const params = [];
  const where = ["c.estado = 'activo'", 'c.deleted_at IS NULL'];
  const visibilityClause = buildClientVisibilityClause(params, clientVisibility, 'c');
...
function buildPricingProductSelect() {
  return `SELECT p.id, p.nombre, p.categoria_id, p.proveedor_id,
...
async function listProductsForPricing({
  productIds = [],
  categoryId = null,
  ...
  if (Number.isInteger(categoryIdNum) && categoryIdNum > 0) {
    const categoryIds = await categoryRepo.getCategoryFilterIds(categoryIdNum, {
      includeDescendants: Boolean(includeDescendants),
      onlyActive: true,
    });
    if (!categoryIds.length) return [];
    const placeholders = categoryIds.map((_, i) => `$${params.length + i + 1}`).join(', ');
    where.push(`p.categoria_id IN (${placeholders})`);
    params.push(...categoryIds);
  }

  params.push(lim);
  const { rows } = await query(
    `${buildPricingProductSelect()}
      WHERE ${where.join(' AND ')}
      ORDER BY p.id ASC
      LIMIT $${params.length}`,
    params
...
async function findPromiseById(id) {
  const promiseId = Number(id);
  if (!Number.isInteger(promiseId) || promiseId <= 0) return null;
  const { rows } = await query(
