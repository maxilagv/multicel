...
const { buildClientVisibility, filterVisibleClientIds } = require('../lib/clientVisibility');
const { getRequestRole, isGlobalRole } = require('../lib/depositoScope');
...
async function riskRanking(req, res) {
  try {
    ...
    const persistRequested = String(req.query.persist || '1') !== '0';
    const clientVisibility = await buildClientVisibility(req);
    const rows = await ownerService.buildRiskRanking({
      limit,
      persistSnapshot: isGlobalRole(role) ? persistRequested : false,
      clientVisibility,
    });
    res.json(rows);
  } catch (e) {
    res.status(e.status || 500).json({ error: e.message || 'No se pudo calcular el ranking de riesgo' });
  }
}
...
async function autoReminders(req, res) {
  try {
    ...
    res
      .status(e.status || 500)
      .json({ error: e.message || 'No se pudieron generar recordatorios automaticos' });
  }
}
...
async function listReminders(req, res) {
  try {
    ...
    res.status(e.status || 500).json({ error: e.message || 'No se pudieron obtener recordatorios' });
  }
}
...
async function listPromises(req, res) {
  try {
    ...
    res.status(e.status || 500).json({ error: e.message || 'No se pudieron obtener promesas' });
  }
}
...
async function updatePromiseStatus(req, res) {
  if (!handleValidation(req, res)) return;
  try {
    const promise = await ownerRepo.findPromiseById(Number(req.params.id));
    if (!promise) return res.status(404).json({ error: 'Promesa no encontrada' });
    const visibleIds = await filterVisibleClientIds(req, [promise.cliente_id]);
    if (!visibleIds.length) {
      return res.status(403).json({ error: 'No tienes permisos para operar sobre este cliente' });
    }
    ...
    res.status(e.status || 500).json({ error: e.message || 'No se pudo actualizar la promesa' });
  }
}
...
const validatePricingScope = [
  ...
  body().custom((_, { req }) => {
    ...
    if (Number.isInteger(categoryId) && categoryId > 0) return true;
    throw new Error('Debe indicar productos o una categoria');
  }),
];

const validateRepricingPreview = [
  ...validatePricingScope,
];

const validateBulkPrice = [
  ...validatePricingScope,
  body('precio_venta').optional({ nullable: true }).isFloat({ min: 0 }),
  body('precio_local').optional({ nullable: true }).isFloat({ min: 0 }),
  body('precio_distribuidor').optional({ nullable: true }).isFloat({ min: 0 }),
  body('precio_final').optional({ nullable: true }).isFloat({ min: 0 }),
  body().custom((_, { req }) => {
    const hasAnyPrice = ['precio_venta', 'precio_local', 'precio_distribuidor', 'precio_final'].some(
      (field) => req.body?.[field] !== undefined && req.body?.[field] !== null && req.body?.[field] !== ''
    );
    if (!hasAnyPrice) {
      throw new Error('Debe indicar al menos un precio');
    }
    return true;
  }),
];
...
async function repricingPreview(req, res) {
  ...
  try {
    const preview = await ownerService.buildRepricingPreview({
      ...
      includeDescendants: Boolean(req.body.include_descendants),
...
async function bulkPricePreview(req, res) {
  if (!handleValidation(req, res)) return;
  try {
    const preview = await ownerService.buildBulkPricePreview({
      productIds: req.body.product_ids || [],
      categoryId: req.body.category_id || null,
      includeDescendants:
        typeof req.body.include_descendants === 'undefined'
          ? true
          : Boolean(req.body.include_descendants),
      limit: Number(req.body.limit || 500),
      prices: {
        precio_venta: req.body.precio_venta,
        precio_local: req.body.precio_local,
        precio_distribuidor: req.body.precio_distribuidor,
        precio_final: req.body.precio_final,
      },
    });
    res.json(preview);
  } catch (e) {
    res.status(500).json({ error: 'No se pudo generar preview de precios masivos' });
  }
}

async function bulkPriceApply(req, res) {
  if (!handleValidation(req, res)) return;
  try {
    const out = await ownerService.applyBulkPrice({
      productIds: req.body.product_ids || [],
      categoryId: req.body.category_id || null,
      includeDescendants:
        typeof req.body.include_descendants === 'undefined'
    ...
    res.status(500).json({ error: 'No se pudieron aplicar precios masivos' });
  }
}
...
module.exports = {
  ...
  bulkPricePreview: [...validateBulkPrice, bulkPricePreview],
  bulkPriceApply: [...validateBulkPrice, bulkPriceApply],
  commandCenter,
  listAlerts,
