...
async function resolveDepositoId(client, depositoId, options = {}) {
  const n = Number(depositoId);
  if (Number.isInteger(n) && n > 0) return n;
  if (options.requireExplicit) {
    const e = new Error('deposito_id es obligatorio para esta operacion');
    e.status = 400;
    e.code = 'DEPOSITO_REQUIRED';
    throw e;
  }
...
async function removeStockTx(client, { producto_id, cantidad, motivo, referencia, usuario_id = null, deposito_id = null }) {
  ...
  const { rows } = await client.query(
    'SELECT cantidad_disponible FROM inventario_depositos WHERE producto_id = $1 AND deposito_id = $2',
    [producto_id, depId]
  );
...
async function reserveStock({ producto_id, cantidad, referencia, usuario_id = null, deposito_id = null }) {
  return withTransaction(async (client) => {
    ...
    const { rows } = await client.query(
      'SELECT cantidad_disponible, cantidad_reservada FROM inventario_depositos WHERE producto_id = $1 AND deposito_id = $2',
      [producto_id, depId]
    );
...
async function transferStockBatch({
  items,
  deposito_origen_id,
  deposito_destino_id,
  motivo,
  ...
  for (const item of Array.isArray(items) ? items : []) {
    ...
    mergedItems.set(productoId, (mergedItems.get(productoId) || 0) + cantidad);
  }

  if (!mergedItems.size) {
    const error = new Error('Debes indicar al menos un producto para transferir');
    error.status = 400;
    throw error;
  }
  ...
  return withTransaction(async (client) => {
    ...
    if (fromDepId === toDepId) {
      const error = new Error('El deposito origen y destino deben ser distintos');
      error.status = 400;
      throw error;
    }

    for (const [productoId, cantidad] of mergedItems.entries()) {
      await ensureInventarioDeposito(client, productoId, fromDepId);
      await ensureInventarioDeposito(client, productoId, toDepId);

      const { rows } = await client.query(
        'SELECT cantidad_disponible FROM inventario_depositos WHERE producto_id = $1 AND deposito_id = $2',
        [productoId, fromDepId]
      );
      const available = rows[0]?.cantidad_disponible ?? 0;
      if (available < cantidad) {
        const error = new Error(
          `Stock insuficiente para transferencia desde deposito ${fromDepId}`
        );
        error.status = 409;
        throw error;
      }
    ...
    for (const [productoId, cantidad] of mergedItems.entries()) {
      ...
      await client.query(
        `UPDATE inventario_depositos
            SET cantidad_disponible = cantidad_disponible + $1,
                actualizado_en = NOW()
          WHERE producto_id = $2 AND deposito_id = $3`,
        [cantidad, productoId, toDepId]
      );

      await client.query(
        `INSERT INTO movimientos_stock(producto_id, deposito_id, tipo, cantidad, motivo, referencia, usuario_id)
         VALUES ($1, $2, 'salida', $3, $4, $5, $6)`,
        [productoId, fromDepId, cantidad, movMotivo, ref, usuario_id]
      );
      await client.query(
        `INSERT INTO movimientos_stock(producto_id, deposito_id, tipo, cantidad, motivo, referencia, usuario_id)
         VALUES ($1, $2, 'entrada', $3, $4, $5, $6)`,
        [productoId, toDepId, cantidad, movMotivo, ref, usuario_id]
      );
      ...
      audit.log({
        ...
        descripcion: `${cantidad} de dep=${fromDepId} a dep=${toDepId} ref=${ref || ''}`,
      });
    }
  });
}
